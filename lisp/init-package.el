...
(declare-function danis-test-package-archives "init-funcs")

;; At first startup
(when (and (not (file-exists-p custom-file))
           (file-exists-p danis-custom-example-file))
  ;; Copy template
  ...
  (set-package-archives (danis-test-package-archives 'nochart)))
